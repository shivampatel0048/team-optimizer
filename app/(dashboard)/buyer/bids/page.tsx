...
import { Check, X, Clock, MessageCircle, ChevronsDown, Phone } from "lucide-react";
    ...
    const activeBids = [
        {
            ...
            previousDeals: 5,
            bidStatus: 'Pending',
            estimatedDelivery: '25 May, 2025',
            message: `I'm interested in your wheat crop.Can you deliver to Delhi?`
        },
        {
            id: 2,
            crop: 'Basmati Rice',
            ...
            previousDeals: 8,
            bidStatus: 'Pending',
            estimatedDelivery: '28 May, 2025',
            message: 'Looking for high-quality Basmati for our restaurant chain. Can provide regular orders if quality meets expectations.'
        },
    ];
        ...
        <DashboardLayout sidebar={<BuyerSidebar />}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    ...
                    <div className="mt-2 flex items-center">
                        <span className="text-3xl font-bold text-[#386641]">{activeBids.length}</span>
                    </div>
                </div>
                <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            ...
            <div className="border-b border-gray-200 mb-6">
                <nav className="flex space-x-8" aria-label="Tabs">
                    {['active', 'accepted', 'rejected'].map((tab) => (
                        <button
                            key={tab}
                                ...
                                <div className="p-6 flex-grow">
                                    <div className="flex flex-col md:flex-row md:justify-between mb-4">
                                        ...
                                        <div className="mt-2 md:mt-0">
                                            <span className="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-1 rounded flex items-center w-fit">
                                                <Clock className="h-3 w-3 mr-1" /> {bid.bidStatus}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            ...
                                            <p className="text-sm font-medium">{bid.farmer}</p>
                                            <p className="text-xs text-gray-500">{bid.farmerLocation}</p>
                                            <div className="flex items-center mt-1">
                                                <div className="flex items-center">
                                                    {Array(5).fill(0).map((_, i) => (
                                                        <svg key={i} className={`w-3 h-3 ${i < Math.floor(bid.farmerRating) ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                                        </svg>
                                                    ))}
                                                    <span className="text-xs font-medium text-gray-700 ml-1">{bid.farmerRating}</span>
                                                </div>
                                        ...
                                        <div>
                                            <p className="text-xs text-gray-500">Bid Details</p>
                                            <div className="flex items-center space-x-2">
                                                <div>
                                                    <p className="text-xs text-gray-500">Asking price</p>
                                                    <p className="text-sm font-medium">{bid.askingPrice}</p>
                                                </div>
                                            ...
                                            <div className="mt-2">
                                                <p className="text-xs text-gray-500">Est. delivery: {bid.estimatedDelivery}</p>
                                                <p className="text-xs text-gray-500">Bid placed {bid.bidDate}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {bid.message && (
                                        <div className="bg-gray-50 p-3 rounded-lg mt-2 flex">
                                            ...
                                            <div>
                                                <p className="text-xs font-medium text-gray-500">Your message:</p>
                        ...
                        <div key={bid.id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                                    ...
                                    <div className="flex flex-col md:flex-row md:justify-between mb-4">
                                        ...
                                        <div className="mt-2 md:mt-0">
                                            <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-1 rounded flex items-center w-fit">
                                                <Check className="h-3 w-3 mr-1" /> {bid.bidStatus}
                                            </span>
                                        </div>
                                    </div>
                                        ...
                                        <div>
                                            <div className="flex justify-between">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm font-medium">{bid.estimatedDelivery}</p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-500">Payment</p>
                                                    <p className={`text-sm font-medium ${bid.paymentStatus === 'Paid' ? 'text-green-600' : 'text-orange-600'}`}>
                                                        {bid.paymentStatus}
                                                    </p>
                                                </div>
                                            </div>
                                            <p className="text-xs text-gray-500 mt-2">Accepted on {bid.acceptedDate}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="bg-gray-50 p-4 flex justify-between">
                                <div className="text-sm text-gray-600">
                                    Status: <span className="font-medium">Waiting for delivery</span>
                                </div>
                                <a href={`tel:${bid.phone}`} className="inline-flex items-center px-4 py-2 bg-[#386641] text-white rounded-md text-sm font-medium hover:bg-[#294D25]">
                                    <Phone className="h-4 w-4 mr-1" /> Call Farmer
                                </a>
                            </div>
                        </div>
                    ))}
                                ...
                                <div className="p-6 flex-grow">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 mt-2">Rejected on {bid.rejectedDate}</p>
                                        </div>
                                    </div>

                                    <div className="mt-4">
                                        <p className="text-xs text-gray-500">Rejection reason:</p>
                                        <p className="text-sm text-gray-700">{bid.rejectionReason}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="bg-gray-50 p-4 flex justify-end">
                                <button className="inline-flex items-center px-4 py-2 bg-[#A7C957] text-[#294D25] rounded-md text-sm font-medium hover:bg-[#8FB944]">
                                    Place New Bid
                                </button>
                            </div>
                        </div>
